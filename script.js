...
async function conectarSupabase() {
    ...
    try {
        ...
        if (typeof window.supabase === 'undefined') {
            throw new Error('La librería de Supabase no se cargó correctamente');
        }
...
function limpiarFiltros() {
    // Marcar solo filtros de acceso
    document.getElementById('filterMaritimo').checked = true;
    document.getElementById('filterTerrestre').checked = true;

    // Desmarcar todos los demás filtros
    const otherFilters = [
        'filterCultural', 'filterNatural', 'filterGastronomico',
        'filterRio', 'filterPlaya', 'filterTourHarbor',
        'filterAlojamiento', 'filterInternet',
        'filterTubing', 'filterCanotaje', 'filterSnorkel', 'filterBuceo',
        'filterBallenas', 'filterCascadas', 'filterCatamaran'
    ];
...
function aplicarFiltros() {
    ...
    filteredSites = allSites.filter(site => {
        ...
        // Si no hay filtros activos en turismo, mostrar todos
        const hasTurismoFilters = Object.values(filters.turismo).some(v => v);
        if (hasTurismoFilters) {
            const turismoMatch = 
                (filters.turismo.cultural && site['Turismo Cultural'] === 'Si') ||
                (filters.turismo.natural && site['Turismo Natural'] === 'Si') ||
                (filters.turismo.gastronomico && site['Turismo Gastronomico'] === 'Si') ||
                (filters.turismo.rio && site['Turismo Rio'] === 'Si') ||
                (filters.turismo.playa && site['Turismo Playa'] === 'Si') ||
                (filters.turismo.tourHarbor && site['Tour Harbor'] === 'Si');
            
            if (!turismoMatch) return false;
        }

        // Filtro de servicios
        const hasServiciosFilters = Object.values(filters.servicios).some(v => v);
        if (hasServiciosFilters) {
            const serviciosMatch = 
                (filters.servicios.alojamiento && site.Alojamiento === 'Si') ||
                (filters.servicios.internet && site['Cn. Internet'] === 'Si');
            
            if (!serviciosMatch) return false;
        }

        // Filtro de actividades
        const hasActividadesFilters = Object.values(filters.actividades).some(v => v);
        if (hasActividadesFilters) {
            const actividadesMatch = 
                (filters.actividades.tubing && site['Tubing / Flotador'] === 'Si') ||
                (filters.actividades.canotaje && site.Canotaje === 'Si') ||
                (filters.actividades.snorkel && site['Snorkel / Caterear'] === 'Si') ||
                (filters.actividades.buceo && site.Buceo === 'Si');
            
            if (!actividadesMatch) return false;
        }

        // Filtro de atractivos
        const hasAtractivosFilters = Object.values(filters.atractivos).some(v => v);
        if (hasAtractivosFilters) {
            const atractivosMatch = 
                (filters.atractivos.ballenas && site['Avista Ballenas'] === 'Si') ||
                (filters.atractivos.cascadas && site.Cascada === 'Si') ||
                (filters.atractivos.catamaran && site.Catamaran === 'Si');
            
            if (!atractivosMatch) return false;
        }
    ...
    mostrarSitiosLista(filteredSites);
    mostrarSitiosMapa(filteredSites);
}

// ============================================
// MOSTRAR SITIOS EN LA LISTA
// ============================================
function mostrarSitiosLista(sites) {
